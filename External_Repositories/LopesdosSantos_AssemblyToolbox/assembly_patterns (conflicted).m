...
%   Members:  matrix that identifies the members of the assembly.
%             Otsu's thresholding method on the absolute weight vector of
%             each assembly.
% 
...
% Modified by Juan Facundo Morici
% Otsu's method for thresholding the absolute weights to determine members
% of an eassemblie.
% Boucly, Pompelli, Todorova, et al 2023
...
%         disp(['The max lambda for detection was ' , num2str(lambda_max)])
    case 'binshuffling'
...
Members = zeros(size(AssemblyTemplates));
for i = 1 : size(AssemblyTemplates,2)
    Members(:,i) = abs(AssemblyTemplates(:,i))>otsuthresh(abs(AssemblyTemplates(:,i)));
    
%     [~,ii] = max(abs(AssemblyTemplates(:,i))); %change the simbol to the one with largr weight
%     if AssemblyTemplates(ii,i) < 0 
%         AssemblyTemplates(:,i) = AssemblyTemplates(:,i) * -1;
%     end
    clear ii
end
