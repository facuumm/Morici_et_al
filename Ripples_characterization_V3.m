...
Rate.dHPC.aversive.pre.mean = [];
Rate.dHPC.aversive.post.mean = [];
Rate.dHPC.reward.pre.mean = [];
Rate.dHPC.reward.post.mean = [];
...
Rate.vHPC.aversive.pre.mean = [];
Rate.vHPC.aversive.post.mean = [];
Rate.vHPC.reward.pre.mean = [];
Rate.vHPC.reward.post.mean = [];

Rate.vHPC.aversive.pre.all = [];
Rate.vHPC.aversive.post.all = [];
Rate.vHPC.reward.pre.all = [];
Rate.vHPC.reward.post.all = [];
...
for tt = 1:length(path)
    ...
    for t = 1 : length(subFolders)-2
        ...
        if D 
            ...
            durations.D.all = [durations.D.all ; nanmean(ripplesD(:,3)-ripplesD(:,1))];
            amplitude.D.all = [amplitude.D.all ; nanmean(ripplesD(:,4))];
            ...
            durations.D.B = [durations.D.B ; nanmean(tmp(:,3)-tmp(:,1))];
            tmp1 = Restrict(ripplesD,NREM.R);
            durations.D.R = [durations.D.R ; nanmean(tmp1(:,3)-tmp1(:,1))];
            tmp2 = Restrict(ripplesD,NREM.A);
            durations.D.A = [durations.D.A ; nanmean(tmp2(:,3)-tmp2(:,1))];

            %Amplitude per conditions
            amplitude.D.B = [amplitude.D.B ; nanmean(tmp(:,4))];
            amplitude.D.R = [amplitude.D.R ; nanmean(tmp1(:,4))];
            amplitude.D.A = [amplitude.D.A ; nanmean(tmp2(:,4))];
            ...
            if aversiveTS(1) < rewardTS(1)
                ...
                Rate.dHPC.aversive.pre.mean = [Rate.dHPC.aversive.pre.mean ; length(x)/sum([NREM.B(:,2)-NREM.B(:,1)])];
                Rate.dHPC.aversive.post.mean = [Rate.dHPC.aversive.post.mean ; length(y)/sum([NREM.A(:,2)-NREM.A(:,1)])];
                clear x y
                ...
                Rate.dHPC.reward.pre.mean = [Rate.dHPC.reward.pre.mean ; length(x)/sum([NREM.A(:,2)-NREM.A(:,1)])];
                Rate.dHPC.reward.post.mean = [Rate.dHPC.reward.post.mean ; length(y)/sum([NREM.R(:,2)-NREM.R(:,1)])];
                ...
                Duration.dHPC.aversive.pre.all = [Duration.dHPC.aversive.pre.all ; [x(:,3) - x(:,1)]];
                Duration.dHPC.aversive.post.all = [Duration.dHPC.aversive.post.all ; [y(:,3) - y(:,1)]];
                Rate.dHPC.aversive.pre.mean = [Rate.dHPC.aversive.pre.mean ; length(x)/sum([NREM.R(:,2)-NREM.R(:,1)])];
                Rate.dHPC.aversive.post.mean = [Rate.dHPC.aversive.post.mean ; length(y)/sum([NREM.A(:,2)-NREM.A(:,1)])];
                clear x y
                ...
                Duration.dHPC.reward.pre.all = [Duration.dHPC.reward.pre.all ; [x(:,3) - x(:,1)]];
                Duration.dHPC.reward.post.all = [Duration.dHPC.reward.post.all ; [y(:,3) - y(:,1)]];
                Rate.dHPC.reward.pre.mean = [Rate.dHPC.reward.pre.mean ; length(x)/sum([NREM.B(:,2)-NREM.B(:,1)])];
                Rate.dHPC.reward.post.mean = [Rate.dHPC.reward.post.mean ; length(y)/sum([NREM.R(:,2)-NREM.R(:,1)])];
                clear x y
            end
        end
        ...
        if V
            ...
            durations.V.all = [durations.V.all ; nanmean(ripplesV(:,3)-ripplesV(:,1))];
            amplitude.V.all = [amplitude.V.all ; nanmean(ripplesV(:,4))];
            ...
            durations.V.B = [durations.V.B ; nanmean(tmp(:,3)-tmp(:,1))];
            tmp1 = Restrict(ripplesV,NREM.R);
            durations.V.R = [durations.V.R ; nanmean(tmp1(:,3)-tmp1(:,1))];
            tmp2 = Restrict(ripplesV,NREM.A);
            durations.V.A = [durations.V.A ; nanmean(tmp2(:,3)-tmp2(:,1))];

            %Amplitude per conditions
            amplitude.V.B = [amplitude.V.B ; nanmean(tmp(:,4))];
            amplitude.V.R = [amplitude.V.R ; nanmean(tmp1(:,4))];
            amplitude.V.A = [amplitude.V.A ; nanmean(tmp2(:,4))];
            ...
            if aversiveTS(1) < rewardTS(1)
                ...
                Rate.vHPC.aversive.pre.mean = [Rate.vHPC.aversive.pre.mean ; length(x)/sum([NREM.B(:,2)-NREM.B(:,1)])];
                Rate.vHPC.aversive.post.mean = [Rate.vHPC.aversive.post.mean ; length(y)/sum([NREM.A(:,2)-NREM.A(:,1)])];                
                clear x y
                ...
                Rate.vHPC.reward.pre.mean = [Rate.vHPC.reward.pre.mean ; length(x)/sum([NREM.A(:,2)-NREM.A(:,1)])];
                Rate.vHPC.reward.post.mean = [Rate.vHPC.reward.post.mean ; length(y)/sum([NREM.R(:,2)-NREM.R(:,1)])];                 
                clear x y
            else
                ...
                Duration.vHPC.aversive.pre.all = [Duration.vHPC.aversive.pre.all ; [x(:,3) - x(:,1)]];
                Duration.vHPC.aversive.post.all = [Duration.vHPC.aversive.post.all ; [y(:,3) - y(:,1)]];
                Rate.vHPC.aversive.pre.mean = [Rate.vHPC.aversive.pre.mean ; length(x)/sum([NREM.R(:,2)-NREM.R(:,1)])];
                Rate.vHPC.aversive.post.mean = [Rate.vHPC.aversive.post.mean ; length(y)/sum([NREM.A(:,2)-NREM.A(:,1)])];                
                clear x y
                ...
                Duration.vHPC.reward.pre.all = [Duration.vHPC.reward.pre.all ; [x(:,3) - x(:,1)]];
                Duration.vHPC.reward.post.all = [Duration.vHPC.reward.post.all ; [y(:,3) - y(:,1)]];
                Rate.vHPC.reward.pre.mean = [Rate.vHPC.reward.pre.mean ; length(x)/sum([NREM.B(:,2)-NREM.B(:,1)])];
                Rate.vHPC.reward.post.mean = [Rate.vHPC.reward.post.mean ; length(y)/sum([NREM.R(:,2)-NREM.R(:,1)])];
                clear x y
            end            
...
%             x = [z ones(length(z),1) ; z1 ones(length(z1),1)*2]; 
%             %ordering of events
%             [i ii] = sort(x(:,1));
%             x = x(ii,:);
%             
...
%             for i = 1 : length(z)
% 
%                 DV = and(z1(:,1) > z(i,1) , z1(:,1) < z(i,2));
%                 VD = and(z1(:,2) > z(i,1) , z1(:,2) < z(i,2));
%                 
%                 if sum(DV)>=1
%                     down = z(i,1);
%                     up = max(max([z1(DV,:) ; z(i,:)]));
%                     bursts.coordinated.DV = [bursts.coordinated.DV ; down up]; clear up down
%                 elseif sum(VD)>=1
%                     down = min(min(z1(VD,:)));
%                     up = max(max([z1(VD,:) ; z(i,:)]));
%                     bursts.coordinated.VD = [bursts.coordinated.VD ; down up]; clear up down
%                 end
%                 
%             end
%             clear z z1
...
%             [x xx] = ConsolidateIntervals(x(:,1:2));
%             % Merge events that are separated by 100ms
%             [bursts.coordinated.all] = merge_events(x, 0.1); clear x xx i ii
%             
...
% Burst Index separated in conditions
figure
subplot(121)
x = [[BurstIndex.D.B ; BurstIndex.D.R ; BurstIndex.D.A] , [ones(length(BurstIndex.D.B),1) ; ones(length(BurstIndex.D.R),1)*2 ; ones(length(BurstIndex.D.R),1)*3]];
scatter(x(:,2),x(:,1),"filled",'jitter','on', 'jitterAmount',0.1),xlim([0 4]),hold on
scatter([1 2 3] , [nanmean(BurstIndex.D.B) nanmean(BurstIndex.D.R) nanmean(BurstIndex.D.A)],'filled'),ylim([0 0.5])
kstest(x(:,1))

x = [BurstIndex.D.B , BurstIndex.D.R , BurstIndex.D.A]; 
figure
[p t stats] = friedman(x,1)
c = multcompare(stats)

subplot(122)
x = [[BurstIndex.V.B ; BurstIndex.V.R ; BurstIndex.V.A] , [ones(length(BurstIndex.V.B),1) ; ones(length(BurstIndex.V.R),1)*2 ; ones(length(BurstIndex.V.R),1)*3]];
scatter(x(:,2),x(:,1),"filled",'jitter','on', 'jitterAmount',0.1),xlim([0 4]),hold on
scatter([1 2 3] , [nanmean(BurstIndex.V.B) nanmean(BurstIndex.V.R) nanmean(BurstIndex.V.A)],'filled'),ylim([0 0.5])
kstest(x(:,1))

x = [BurstIndex.V.B , BurstIndex.V.R , BurstIndex.V.A]; 
figure
[p t stats] = friedman(x,1)
c = multcompare(stats)

grps = [];
grps = [grps,[ones(length(BurstIndex.D.B),1) ; ones(length(BurstIndex.D.R),1)*2 ; ones(length(BurstIndex.D.R),1)*3 ; [ones(length(BurstIndex.V.B),1) ; ones(length(BurstIndex.V.R),1)*2 ; ones(length(BurstIndex.V.R),1)*3]]];
grps = [grps , [ones(40*3,1) ; ones(47*3,1)*2]];
x = [BurstIndex.D.B ; BurstIndex.D.R ; BurstIndex.D.A ; BurstIndex.V.B ; BurstIndex.V.R ; BurstIndex.V.A];

figure
[p t stats] = anovan(x , grps,'model','interaction','varnames',{'condition','structure'})
...
x = [x1  ; x3 ; x4  ; x6];
x = [x , [ones(40,1) ; ones(40,1)*2 ; ones(40,1)*3 ; ones(40,1)*4 ]];
x = [x , [ones(40,1) ; ones(40,1) ; ones(40,1)*2 ; ones(40,1)*2]];
x = [x , [ones(40,1) ; ones(40,1)*2 ; ones(40,1) ; ones(40,1)*2]];

figure
scatter(x(:,2),x(:,1),"filled",'jitter','on', 'jitterAmount',0.1),xlim([0 7]),hold on
scatter([1 2 3 4] , [nanmean(x1) nanmean(x3) nanmean(x4) nanmean(x6)],'filled')%,ylim([0 0.4])
boxplot(x(:,1),x(:,2)),ylim([0 60])
