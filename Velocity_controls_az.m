...
for tt = 1:length(path)
    ...
    for t = 1 : length(subFolders)-2
        ...
        bin_velocity_rew=[bin_limits', bin_velocity_rew];
        clear strt stop bin_limits ini fin status
       ...
       %Check lenght of in_maze segments to define real laps
        ...
        try
            ...
            for d=1:size(group_vHPC,1)
                ...
                vHPC_all = [vHPC_all; between,1;withinA,2;withinR,3];
...
for a=1:size(ylabels,2)
    ...
    subplot(1,3,a); hold on; 
    x = dHPC_all(:,6);
    y= dHPC_all(:,p);%Change the column number (1-4) to choose which variable to plot 
    c = [.3,.3,.3];
    scatter(x,y,30,c,"filled",'jitter','on', 'jitterAmount',0.3); xlim([0 4]);
    ylabel(ylabels{a});
    xticks([1 2 3])
    xticklabels({'Bet', 'WA', 'WR'});
    hold on
    s1=scatter(1,nanmedian(dHPC_all(dHPC_all(:,6)==1,p)), "filled");s1.MarkerFaceColor = [0 0 0];
    s2=scatter(2,nanmedian(dHPC_all(dHPC_all(:,6)==2,p)), "filled");s2.MarkerFaceColor = [1 0.1 0.2];
    s3=scatter(3,nanmedian(dHPC_all(dHPC_all(:,6)==3,p)), "filled");s3.MarkerFaceColor = [0.1 0.3 1];
end
...
for a=1:size(ylabels,2)
    ...
    subplot(1,3,a); hold on; 
    x = vHPC_all(:,6);
    y= vHPC_all(:,p);%Change the column number (1-4) to choose which variable to plot 
    c = [.3,.3,.3];
    scatter(x,y,30,c,"filled",'jitter','on', 'jitterAmount',0.3); xlim([0 4]);
    ylabel(ylabels{a});
    xticks([1 2 3])
    xticklabels({'Bet', 'WA', 'WR'});
    hold on
    s1=scatter(1,nanmedian(vHPC_all(vHPC_all(:,6)==1,p)), "filled");s1.MarkerFaceColor = [0 0 0];
    s2=scatter(2,nanmedian(vHPC_all(vHPC_all(:,6)==2,p)), "filled");s2.MarkerFaceColor = [1 0.1 0.2];
    s3=scatter(3,nanmedian(vHPC_all(vHPC_all(:,6)==3,p)), "filled");s3.MarkerFaceColor = [0.1 0.3 1];
end

%% LAPS NORMALIZD BY VELOCITY 
        ...
        try
            ...
            temp_id= ismember(group_dHPC(:,1),dhpc_sub); group_dHPC=group_dHPC(temp_id,:);
             ...
             for ii=1:size(group_dHPC,1)
                ...
                %Restrict spk and position to laps and movement periods
                spks_ave = Restrict(spks_ave,in_lapA);spks_ave = Restrict(spks_ave , movement.aversive); % Restrict spikes to movement periods and laps
                pos_ave=pos_ave(:,1:2); % removing 3rd column with velocity    
               
                % --- Reward ---
                spks_rew = spks; 
                %Restrict spk and position to laps and movement periods
                spks_rew = Restrict(spks_rew,in_lapR); spks_rew = Restrict(spks_rew, movement.reward); % Restrict to movement periods
                pos_rew=pos_rew(:,1:2); % removing 3rd column with velocity    
                ...
                % and caluclate within/between comparisons 
                        ...
                        if size(in_lapR,1)> size(in_lapA,1) % if more laps in rew
                            ...
                            [within_mean1,within_mean2,between] = pc_parameters_laps(n_lap,pos_rew,spks_rew,in_lapR,pos_ave,spks_ave,in_lapA,sigma,Xedges,'velocity_norm_ave',spbin_velocity_ave,'velocity_norm_rew',spbin_velocity_rew);
                            withinA = within_mean2;
                            withinR = within_mean1;
                            disp('rew');
                        elseif size(in_lapR,1)< size(in_lapA,1)  %if more laps in ave
                            n_lap = size(in_lapR,1); 
                            [within_mean1,within_mean2,between] = pc_parameters_laps(n_lap,pos_ave,spks_ave,in_lapA,pos_rew,spks_rew,in_lapR,sigma,Xedges,'velocity_norm_ave',spbin_velocity_ave,'velocity_norm_rew',spbin_velocity_rew);                            
                            withinA = within_mean1;
                            withinR = within_mean2;
                            disp('ave');
                        else % equal # laps in cond 
                            [withinA] = Within_lap_random(pos_ave,spks_ave,in_lapA,sigma,Xedges,'velocity_norm',spbin_velocity_ave);
                            [withinR] = Within_lap_random(pos_rew,spks_rew,in_lapR,sigma,Xedges,'velocity_norm',spbin_velocity_rew);
                            [between] = Between_lap_random(pos_ave,spks_ave,in_lapA,pos_rew,spks_rew,in_lapR,sigma,Xedges,'velocity_norm_ave',spbin_velocity_ave,'velocity_norm_rew',spbin_velocity_rew);
                            disp('equal');
                        end 
...
for a=1:size(ylabels,2)
    ...
    y= dHPC_all(:,p);%Change the column number (1-4) to choose which variable to plot 
    c = [.3,.3,.3];
    scatter(x,y,20,c,"filled",'jitter','on', 'jitterAmount',0.3); xlim([0 4]);
    ylabel(ylabels{a});
    xticks([1 2 3])
    xticklabels({'Bet', 'WA', 'WR'});
    hold on
...
%Prepare data to test
p = 4; % 1=sp corr 2=fr change 3=overlap 4=pf shift
data=[dHPC_all(dHPC_all(:,6)==1,p), dHPC_all(dHPC_all(:,6)==2,p), ...
    dHPC_all(dHPC_all(:,6)==3,p), dHPC_all(dHPC_all(:,6)==4,p),dHPC_all(dHPC_all(:,6)==5,p)];
data(any(isnan(data), 2), :) = [];
...
%Prepare data to test
p = 4; % 1=sp corr 2=fr change 3=overlap 4=pf shift
data=[vHPC_all(vHPC_all(:,6)==1,p), vHPC_all(vHPC_all(:,6)==2,p), ...
    vHPC_all(vHPC_all(:,6)==3,p), vHPC_all(vHPC_all(:,6)==4,p),vHPC_all(vHPC_all(:,6)==5,p)];
data(any(isnan(data), 2), :) = [];
...
%% FIRING CURVE LAPS NORMALIZD BY VELOCITY 
            ...
            temp_id= ismember(group_dHPC(:,1),dhpc_sub); group_dHPC=group_dHPC(temp_id,:);
             ...
             for ii=1:size(group_dHPC,1)
                ...
                % and caluclate within/between comparisons 
                        ...
                        if size(in_lapR,1)> size(in_lapA,1) % if more laps in rew
                            n_lap = size(in_lapA,1); 
                            [curveR] = pc_firigcurve_laps(n_lap,pos_rew,spks_rew,in_lapR,sigma,Xedges,...
                                'norm1',spbin_velocity_rew);
                            [curveA] = FiringCurve(pos_ave, spks_ave , 'smooth' , sigma , 'nBins' , Xedges , 'minSize' , 4 , 'minPeak' , 0.2,'minTime',0.15);
                            curveA = curveA.rate./spbin_velocity_ave;
                            
                        elseif size(in_lapR,1)< size(in_lapA,1)  %if more laps in ave
                            n_lap = size(in_lapR,1); 
                            [curveA] = pc_firigcurve_laps(n_lap,pos_ave,spks_ave,in_lapA,sigma,Xedges,...
                              'norm1',spbin_velocity_ave);                            
                            [curveR] = FiringCurve(pos_rew, spks_rew , 'smooth' , sigma , 'nBins' , Xedges , 'minSize' , 4 , 'minPeak' , 0.2,'minTime',0.15);
                            curveR = curveR.rate./spbin_velocity_rew;
                    ...
                    dHPC_all = [dHPC_all; curveA,1;curveR,2];
             ...
             for ii=1:size(group_vHPC,1)
                ...
                % and caluclate within/between comparisons 
                 if size(in_lapR,1)> size(in_lapA,1) % if more laps in rew
                    ...
                    vHPC_all = [vHPC_all; curveA,1;curveR,2];
...
for d=1:size(dHPC_all)
    
    if dHPC_all(d,end)==1
       A = dHPC_all(d,1:60) - min(dHPC_all(d,1:60));A = A ./ max(A);
       dhpc_ave = [dhpc_ave;A]; 
    
    elseif dHPC_all(d,end)==2
        A = dHPC_all(d,1:60) - min(dHPC_all(d,1:60));A = A ./ max(A);
       dhpc_rew = [dhpc_rew;A]; 
    end 
...
for d=1:size(vHPC_all)
    
    if vHPC_all(d,end)==1
       A =vHPC_all(d,1:60) - min(vHPC_all(d,1:60));A = A ./ max(A);
       vhpc_ave = [vhpc_ave;A]; 
