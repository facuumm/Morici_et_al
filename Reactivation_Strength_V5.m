...
W = 'CB'; % to select what kind of ripples I want to check
...
for tt = 2:length(path)
    ...
    for t = 1 : length(subFolders)-2
        ...
        bursts.coordinated(or((bursts.coordinated(:,2)-bursts.coordinated(:,1))<0.08 , (bursts.coordinated(:,2)-bursts.coordinated(:,1))>1),:) = [];
        ripple_bursts.baseline = Restrict(bursts.coordinated,NREM.baseline);
        ripple_bursts.reward = Restrict(bursts.coordinated,NREM.reward);
        ripple_bursts.aversive = Restrict(bursts.coordinated,NREM.aversive);        
        clear B bursts
