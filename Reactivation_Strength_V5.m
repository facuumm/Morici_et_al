                    ...
                    if sum(cond.both.aversive)>=1
                        if and(not(strcmp(W,'V')) , not(strcmp(W,'D'))) % check if is not uncoordinated events
                            ...
                            for i = 1:size(R,1)
                                ...
                                tmp = [tmp ; size(Shocks_filt,1) , x , ((aversiveTS_run(2) - aversiveTS_run(1))/1000) ,  sum([movement.aversive(:,2) - movement.aversive(:,1)])];
                            end
                    ...
                    if sum(cond.both.reward)>=1
                        if and(not(strcmp(W,'V')) , not(strcmp(W,'D')))
                            ...
                            for i = 1:size(R,1)
                                ...
                                tmp = [tmp ; size(Rewards_filt,1) , x , ((rewardTS_run(2) - rewardTS_run(1))/1000) , sum([movement.reward(:,2) - movement.reward(:,1)])];
                            end                            
...
%% %% Plot Correlations
figure
% scatter(reactivation.reward.dvHPC(:,12) , reactivation.reward.dvHPC(:,1),'filled','b'),hold on%,ylim([-0.5 0.5]),xlim([0 50]),ylim([-1 1])
scatter(reactivation.aversive.dvHPC(:,12) , reactivation.aversive.dvHPC(:,1),'filled','b'),hold on%,ylim([-0.5 0.5]),xlim([0 50]),ylim([-1 1])
xlim([800 2100])
ylim([-0.6 0.6])
ylabel('Peak Reactivation Srength'),
xlabel('Total Time')

figure
% fitlm(reactivation.reward.dvHPC(:,12) , reactivation.reward.dvHPC(:,1))
fitlm(reactivation.aversive.dvHPC(:,12) , reactivation.aversive.dvHPC(:,1))
plot(ans),
xlim([800 2010])
ylim([-0.6 0.6])
ylabel('Peak Reactivation Srength'),
...
% Active / total
figure
scatter(reactivation.reward.dvHPC(:,13)./reactivation.reward.dvHPC(:,12) , reactivation.reward.dvHPC(:,1),'filled','b'),hold on%,ylim([-0.5 0.5]),xlim([0 50]),ylim([-1 1])
% scatter(reactivation.aversive.dvHPC(:,13)./reactivation.aversive.dvHPC(:,12) , reactivation.aversive.dvHPC(:,1),'filled','b'),hold on%,ylim([-0.5 0.5]),xlim([0 50]),ylim([-1 1])
xlim([0 0.5])
ylim([-0.6 0.6])
ylabel('Peak Reactivation Srength'),
xlabel('Active Time / Total Time')

figure
fitlm(reactivation.reward.dvHPC(:,13)./reactivation.reward.dvHPC(:,12) , reactivation.reward.dvHPC(:,1))
% fitlm(reactivation.aversive.dvHPC(:,13)./reactivation.aversive.dvHPC(:,12) , reactivation.aversive.dvHPC(:,1))
plot(ans),
xlim([0 0.5])
ylim([-0.6 0.6])
ylabel('Peak Reactivation Srength'),
