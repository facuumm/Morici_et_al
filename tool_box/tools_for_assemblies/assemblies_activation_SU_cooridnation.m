function [Pre Post T I] = assemblies_activation_SU_cooridnation(path)
        ...
        if RB
            if or(numberD > 3 , numberV > 3)
                ...
                if isfile('dorsalventral_assemblies_aversiveVF.mat')
                    ...
                    th = Thresholded.aversive(:,cond.both);
                    ...
                    for j = 1 : size(th,2)
                        ...
                        for i = 1 : size(clusters.dHPC,1)
                            if th(i,j)
                                %                             break
                                member1 = clusters.all(i);
                                for ii = 1 : size(clusters.vHPC,1)
                                    if th(ii+size(clusters.dHPC,1),j)
                                        %                                     break
                                        member2 = clusters.all(ii+size(clusters.dHPC,1));
                                        
                                        % Pre
                                        x = Restrict(spks(spks(:,1)==member1,2) , TS.pre);
                                        y = Restrict(spks(spks(:,1)==member2,2) , TS.pre);
                                        ...
                                        if and(not(isempty(ccg1)) ,  not(isempty(ccg2)))
                                            Pre.aversive.members = [Pre.aversive.members , ccg1];
                                            Post.aversive.members = [Post.aversive.members , ccg2];
                                            
                                            I.aversive = [I.aversive ; R(j,1)]; clear p
                                        end
                                    end
                                end
                            end
                ...
                if isfile('dorsalventral_assemblies_rewardVF.mat')
                    ...
                    th = Thresholded.reward(:,cond.both);
                            ...
                            if th(i,j)
                                ...
                                for ii = 1 : size(clusters.vHPC,1)
                                    if th(ii+size(clusters.dHPC,1),j)
                                        ...
                                        if and(not(isempty(ccg1)) ,  not(isempty(ccg2)))
                                            Pre.reward.members = [Pre.reward.members , ccg1];
                                            Post.reward.members = [Post.reward.members , ccg2];
                                            
                                            I.reward = [I.reward ; R(j,1)]; clear p
                                        end
                                    end
                                end
                            end
