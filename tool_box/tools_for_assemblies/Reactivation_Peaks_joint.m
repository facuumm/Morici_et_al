...
for i = 1 : size(Peaks.aversive.Post,2)
    ...
    if and(not(isempty(post)),not(isempty(pre)))
        % shorten data
        n = length(pre);     m = length(post);
        if n>m
            pre = pre(1:m);
        elseif n<m
            post=post(1:n);
        ...
        diffA.one = [diffA.one; (nanmean(x))]; clear pre post n m
    end
    
    if and(not(isempty(post2)),not(isempty(pre2)))
        % shorten data
        n = length(pre2);     m = length(post2);
        if n>m
            pre2 = pre2(1:m);
        elseif n<m
            post2 = post2(1:n);
        ...
        x = post2./pre2; x(x==inf) = [];
        
        diffA.two = [diffA.two; (nanmean(x))]; clear pre2 post2 n m
    end
    
    x = tmp./tmp1; x(x==inf) = [];
    diffA.all = [diffA.all ; nanmean(x)]; clear x
...
cumR2 = [];  cumR2P = [];
DurationR = [];
ReactR.post = nan(40,51);
ReactR.pre = nan(40,51);
diffR.one = [];
diffR.two = [];
diffR.all = [];
for i = 1 : size(Peaks.reward.Post,2)
    ...
    if and(not(isempty(post)),not(isempty(pre)))
        ...
        x = tmp./tmp1; x(x==inf) = [];
        diffR.one = [diffR.one; (nanmean(x))]; clear pre post n m
    end
    ...
    if and(not(isempty(post2)),not(isempty(pre2)))
        ...
        diffR.two = [diffR.two; (nanmean(x))]; clear pre2 post2 n m
    end    
    
    x = tmp./tmp1; x(x==inf) = [];
    diffR.all = [diffR.all ; nanmean(x)]; clear x
    end
end

p_value = compare_ks_distance(cumA , cumAP , cumR , cumRP , 1000)
