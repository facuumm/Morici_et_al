function [R] = reactivation_strength_restricted(patterns , cond , SpikeTrain , Is , th , type , config , normalization, templates)
...
for i = 1:size(a,1)
    ...
    [pks.baseline,loc.b] = findpeaks(a(i,Is.baseline),bins(Is.baseline),'MinPeakHeight',th);
    [pks.reward,loc.r] = findpeaks(a(i,Is.reward),bins(Is.reward),'MinPeakHeight',th);
    [pks.aversive,loc.a] = findpeaks(a(i,Is.aversive),bins(Is.aversive),'MinPeakHeight',th);
...
%     m = nanmean(surrogated);
    ...
    if type == 'A' % check if is aversive assembly
        if config == 1
            if normalization
                Post = shortenIntervals(Is.timestamps.sleep.aversive, 3600, false);
                Pre = shortenIntervals(Is.timestamps.sleep.baseline, 3600, true);
                Post = Restrict([loc.a pks.aversive'] , Post);
                Pre = Restrict([loc.b pks.baseline'] , Pre);            
                if and(size(Pre,1)>1,size(Post,1)>1)
                    strength = (nanmean(Post(:,2)) - nanmean(Pre(:,2)))/(nanmean([Post(:,2) ; Pre(:,2)])); clear Post Pre%      
                else
                    strength = nan;
                end                
...
%                 strength1 = (size(p1,1)/(sum(tmp1(:,2)-tmp1(:,1))/60)) - (size(p2,1)/(sum(tmp2(:,2)-tmp2(:,1))/60));
            ...
            if normalization
                Post = shortenIntervals(Is.timestamps.sleep.aversive, 3600, false);
                Pre = shortenIntervals(Is.timestamps.sleep.reward, 3600, true);                
                Post = Restrict([loc.a pks.aversive'] , Post);
                Pre = Restrict([loc.r pks.reward'] , Pre);
                if and(size(Pre,1)>1,size(Post,1)>1)
                    strength = (nanmean(Post(:,2)) - nanmean(Pre(:,2)))/(nanmean([Post(:,2) ; Pre(:,2)])); clear Post Pre%      
                else
                    strength = nan;
...
%                 strength1 = (size(p1,1)/(sum(tmp1(:,2)-tmp1(:,1))/60)) - (size(p2,1)/(sum(tmp2(:,2)-tmp2(:,1))/60));
    ...
    elseif type == 'R' % check if is reward assembly
        if config == 2
            if normalization
                Post = shortenIntervals(Is.timestamps.sleep.reward, 3600, false);
                Pre = shortenIntervals(Is.timestamps.sleep.baseline, 3600, true);                
                Post = Restrict([loc.r pks.reward'] , Post);
                Pre = Restrict([loc.b pks.baseline'] , Pre);            
                if and(size(Pre,1)>1,size(Post,1)>1)
                    strength = (nanmean(Post(:,2)) - nanmean(Pre(:,2)))/(nanmean([Post(:,2) ; Pre(:,2)])); clear Post Pre%      
                else
                    strength = nan;
                end                %                 strength1 = (length(pks.reward)/timeR - length(pks.baseline)/timeB)/(sum([length(pks.reward) , length(pks.baseline)])/(timeR+timeB));
...
%                 strength1 = (size(p1,1)/(sum(tmp1(:,2)-tmp1(:,1))/60)) - (size(p2,1)/(sum(tmp2(:,2)-tmp2(:,1))/60));
            ...
            if normalization
                Post = shortenIntervals(Is.timestamps.sleep.reward, 3600, false);
                Pre = shortenIntervals(Is.timestamps.sleep.aversive, 3600, true);                   
                Post = Restrict([loc.r pks.reward'] , Post);
                Pre = Restrict([loc.a pks.aversive'] , Pre);         
                if and(size(Pre,1)>1,size(Post,1)>1)
                    strength = (nanmean(Post(:,2)) - nanmean(Pre(:,2)))/(nanmean([Post(:,2) ; Pre(:,2)])); clear Post Pre%      
                else
                    strength = nan;
