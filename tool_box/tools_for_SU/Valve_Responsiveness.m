...
for tt = 1:length(path)
    ...
    for t = 1 : length(subFolders)-2
        ...
        if numberD>0
            [c , bins , r] = SU_responsivness_fixed(spks_dHPC,clusters.dHPC,Rewards_filt(:,1),[movement.reward(1,1) movement.reward(end,1)],[0 1],4,0.1,1,'zscore',2);
            curve.dHPC = [curve.dHPC , c];
            responsive.dHPC = [responsive.dHPC , r];
            dHPC_valve.id = clusters.dHPC;
            dHPC_valve.curve = c;
            dHPC_valve.responssiveness = r; clear c r
            save([cd,'\dHPC_valve1.mat'],'dHPC_valve'); clear dHPC_valve
        end
        
        if numberV>0
            [c , bins , r] = SU_responsivness_fixed(spks_vHPC,clusters.vHPC,Rewards_filt(:,1),[movement.reward(1,1) movement.reward(end,1)],[0 1],4,0.1,1,'zscore',2);
            curve.vHPC = [curve.vHPC , c];
            responsive.vHPC = [responsive.vHPC , r];
            vHPC_valve.id = clusters.vHPC;
            vHPC_valve.curve = c;
            vHPC_valve.responssiveness = r; clear c r
            save([cd,'\vHPC_valve1.mat'],'vHPC_valve'); clear vHPC_valve
        end
...
% T = [-5 : 0.1 : 5];
% c = nanmean(curve.dHPC(:,responsive.dHPC==1)');
% plot(T,c)
% 
% hold on
% 
% c1 = nanmean(curve.vHPC(:,responsive.vHPC==1)');
% plot(T,c1)
