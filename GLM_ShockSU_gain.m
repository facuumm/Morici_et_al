function [Pred_shock , Pred_member , Pred_nomember] = GLM_ShockSU_gain(path)
...
%             clear limits events
            ...
            for i = 1 : size(patterns,2)
                ...
                if sum(and(template(:,1) , template(:,2)))>0
                    spike_data.spks = spks;
                    spike_data.clusters = clusters.all;
                    spike_data.cellulartype = cellulartype;
                    spike_data.limits = limits;
                    
                    % Model
                    [member, shock_resp] = train_test_poisson_glm_member_shuffle(spike_data, template, [bins' a(i,:)'], movement.aversive, [ripple_event.aversive(:,1) ripple_event.aversive(:,3)], [numberD , numberD+numberV]);
                    % Saving data  
                    % dHPC
                    if not(isempty(shock_resp.dHPC))
                        Pred_shock.dHPC = [Pred_shock.dHPC ; shock_resp.dHPC];
                    end
                    if not(isempty(member.dHPC))
                        Pred_member.dHPC = [Pred_member.dHPC ; member.dHPC];
                    end                        
                    % vHPC
                    if not(isempty(shock_resp.vHPC))
                        Pred_shock.vHPC = [Pred_shock.vHPC ; shock_resp.vHPC];
                    end
                    if not(isempty(member.dHPC))
                        Pred_member.vHPC = [Pred_member.vHPC ; member.vHPC];
                    end   
                end
                
                clear member shock_resp
            end
        end
        disp(' ')
...
% % 
% % --- dHPC ---
% y1 = Pred_member.dHPC(:,7); y1 = y1(~isnan(y1));
% y2 = Pred_shock.dHPC(:,7);    y2 = y2(~isnan(y2));
% 
...
% % refs = min([length(y1), length(y2)]);
% % 
% % % Random downsampling
% % y1 = y1(randperm(length(y1), refs));
% % y2 = y2(randperm(length(y2), refs));
% 
% matrix1 = [[ones(length(y1),1), y1]; [2*ones(length(y2),1), y2]];
% 
% % --- vHPC ---
% y1 = Pred_member.vHPC(:,7); y1 = y1(~isnan(y1));
% y2 = Pred_shock.vHPC(:,7);    y2 = y2(~isnan(y2));
% 
% % Outlier removal (IQR method)
% y1 = y1(~isoutlier(y1));
% y2 = y2(~isoutlier(y2));
...
% % y2 = y2(randperm(length(y2), refs));
% 
% matrix2 = [[ones(length(y1),1), y1]; [2*ones(length(y2),1), y2]];
% 
% 
% two_way_anova_with_posthoc(matrix1, matrix2)
% yline(0,'--')
% 
% boxplot(matrix1(:,2),matrix1(:,1))
% boxplot(matrix2(:,2),matrix2(:,1))
% 
% 
% %% -------%%
