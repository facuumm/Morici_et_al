...
%%%dHPC%%%%
tempA=[];
for i =1:size(dhpc_shock,1)
    A = dhpc_shock(i,:) - min(dhpc_shock(i,:));
    A = A ./ max(A);
    tempA=[tempA ; A];
    clear A
end
...
title('dHPC Aversive');xline(2,'r');xlabel(['Shock',char(10),'Time bins']); 
ylabel('Sessions');
...
xline(20,'r');xline(30,'r');ylim([0.2 0.5]);
...
for i =1:size(vhpc_shock,1)
    A = vhpc_shock(i,:) - min(vhpc_shock(i,:));
    A = A ./ max(A);
    tempA=[tempA ; A];
    clear A
end
...
title('vHPC Aversive');xline(2,'r');xlabel(['Shock',char(10),'time(sec)']); 
ylabel('Sessions'); 
subplot(2,1,2);hold on; plot([1:1:size(dhpc_shock,2)],nanmean(tempA));
ciplot(nanmean(tempA)-nansem(tempA),nanmean(tempA)+nansem(tempA),[1:1:size(dhpc_shock,2)]); alpha 0.1; hold on;
xline(20,'r');xline(30,'r');ylim([0.2 0.5]);
