...
%% FIRING CURVE SUBSAMPLING BY LAP 
dHPC_all = [];
vHPC_all = [];
bin_size = 1; %sec
             ...
             for ii=1:size(group_dHPC,1)
                ...
                % and caluclate within/between comparisons 
                        ...
                        if size(in_lapR,1)> size(in_lapA,1) % if more laps in rew
                            ...
                            [curveA] = FiringCurve(pos_ave, spks_ave , 'smooth' , sigma , 'nBins', Xedges,...
                                'minSize' , 4 , 'minPeak' , 0.2,'minTime',0.15);
                            curveA=curveA.rate;
                        elseif size(in_lapR,1)< size(in_lapA,1)  %if more laps in ave
                            n_lap = size(in_lapR,1); 
...
for tt = 1:length(path)
    ...
    for t = 1 : length(subFolders)-2
        ...
        try
            ...
            for d=1:size(dHPC,2)
                pc = dHPC{d}; 
                dhpc_corr= [dhpc_corr;pc.between(1),pc.between(3)];
            end
    
        catch 
            disp(['No dHPC file in ',session]);
        end 
         ...
         try
            ...
            for d=1:size(vHPC,2)
                pc = vHPC{d}; 
                vhpc_corr= [vhpc_corr;pc.between(1),pc.between(3)];
            end
...
dhpc_corr((dhpc_corr(:,1)<=-2.5),:)=[];dhpc_corr=abs(dhpc_corr);
dhpc_table = array2table(dhpc_corr,'VariableNames',{'spcorr','overlap'}); 
                mdl = fitlm(dhpc_table)
                R= mdl.Rsquared.Ordinary; 
                figure;plot(mdl)

                
vhpc_table = array2table(vhpc_corr,'VariableNames',{'spcorr','overlap'}); 
...
for tt = 1:length(path)
    ...
    for t = 1 : length(subFolders)-2
       ...
       %Check lenght of in_maze segments to define real laps
        ...
        dhpc_pc.ave=[];dhpc_pc.rew=[];dhpc_pc.both=[];
...
%                 disp(['qr= ', num2str(qr), 'skaggs=' num2str(statsR.specificity)]);
                ...
                if and(and(statsA.specificity>qa,sum(statsA.field(:,:,1))>= 4 ),fr_ave>tresh)
                    dhpc_pc.ave =[dhpc_pc.ave, cluster]; %averesive pc
                end 
                if and(and(statsR.specificity>qr,sum(statsR.field(:,:,1))>= 4 ),fr_rew>tresh)
                    dhpc_pc.rew = [dhpc_pc.rew, cluster];
                end
                if and(and(and(statsA.specificity>qa,sum(statsA.field(:,:,1))>= 4 ),fr_ave>tresh)...
                        ,and(and(statsR.specificity>qr,sum(statsR.field(:,:,1))>= 4 ),fr_rew>tresh))
                     dhpc_pc.both = [dhpc_pc.both, cluster];
                end 
            end
        end
        ...
        disp('vHPC Firing rate map calculation')
        vhpc_pc.ave=[];vhpc_pc.rew=[];vhpc_pc.both=[];
        for ii=1:size(group_vHPC,1)
            ...
            if celltype % check if pyr
                ...
                if and(and(statsA.specificity>qa,sum(statsA.field(:,:,1))>= 4 ),fr_ave>tresh)
                    vhpc_pc.ave =[vhpc_pc.ave, cluster]; %averesive pc
                end
                if and(and(statsR.specificity>qr,sum(statsR.field(:,:,1))>= 4 ),fr_rew>tresh)
                    vhpc_pc.rew = [vhpc_pc.rew, cluster];
                end
                if and(and(and(statsA.specificity>qa,sum(statsA.field(:,:,1))>= 4 ),fr_ave>tresh)...
                        ,and(and(statsR.specificity>qr,sum(statsR.field(:,:,1))>= 4 ),fr_rew>tresh))
                     vhpc_pc.both = [vhpc_pc.both, cluster];
                end 
    ...
    %% Saveing PC INFO 
    if ~isempty(dhpc_pc)
        save([cd,'\dHPC_pc_by_condition.mat'],'dhpc_pc'); 
    end
    if ~isempty(vhpc_pc)
        save([cd,'\vHPC_pc_by_condition.mat'],'vhpc_pc'); 
    end
